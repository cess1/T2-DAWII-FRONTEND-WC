...
public class LoginControllerAsync {
    ...
    public Mono<LoginResponseDTO> autenticar(@RequestBody LoginRequestDTO loginRequestDTO) {
        ...
        if(loginRequestDTO.tipoDocumento() == null || loginRequestDTO.tipoDocumento().trim().length()==0
            ...
            LoginModel loginModel = new LoginModel("01","Error:  Completa correctamente sus credenciales11","");

            return Mono.just(new LoginResponseDTO("01","Error:  Completa correctamente sus credenciales","","",
                    "",""));
        }
        try {
            //Invocar Api
